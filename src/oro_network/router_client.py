...
class RouterClient:
    ...
    async def handle_router_failure(
        ...
        exclude_ids = [router_id] + [
            r_id for r_id, state in failover_states.items() if state.is_in_cooldown() and r_id != router_id
        ]
        ...
        try:
            ...
            for router in routers:
                ...
                try:
                    ...
                    logger.info(
                        f"Rotation complete: {router_id[:16]}... -> {router.router_id[:16]}... (reason: {reason})"
                    )
                    return True
                except Exception as e:
    ...
    def handle_back_pressure(
        ...
        if active:
            ...
            logger.warning(
                f"Router {router_id[:16]}... signaled BACK-PRESSURE: load={load_pct:.1f}%, retry_after={retry_after_ms}ms"  # noqa: E501
            )
        else:
