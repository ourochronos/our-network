...
class RouterNode:
    ...
    async def _release_back_pressure(self, load_pct: float) -> None:
        ...
        logger.info(
            f"Back-pressure RELEASED at {load_pct:.1f}% load (threshold: {self.back_pressure_release_threshold}%)"
        )
    ...
    async def _handle_identify(self, data: dict[str, Any], ws: web.WebSocketResponse) -> str | None:
        ...
        if node_id in self.connection_history:
            ...
            logger.info(
                f"Node reconnected: {node_id[:16]}... (away for {history.time_since_disconnect():.1f}s, queued: {queued_count})"  # noqa: E501
            )
        else:
    ...
    async def stop(self) -> None:
        ...
        for _node_id, conn in list(self.connections.items()):
            try:
    ...
    async def _deliver_circuit_payload(
        ...
        try:
            ...
            if recipient_id in self.connections:
                ...
                self.messages_delivered += 1
                logger.debug(
                    f"Delivered circuit message {message_id[:8] if message_id else 'unknown'}... to {recipient_id[:16]}..."  # noqa: E501
                )
            else:
