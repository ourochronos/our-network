...
class HealthMonitor:
    ...
    def __init__(
        ...
        self._network_baseline: NetworkBaseline | None = None
    ...
    def get_aggregated_health(self, router_id: str) -> float:
        ...
        for _peer_id, peer_data in self._peer_observations.items():
    ...
    def _sample_observations_for_gossip(self) -> list[RouterHealthObservation]:
        ...
        recent_obs = []
        for _router_id, obs in self._own_observations.items():
    ...
    def handle_gossip(self, gossip: HealthGossip) -> None:
        ...
        logger.debug(f"Received gossip from {source[:16]}... with {len(gossip.observations)} observations")
    ...
    def _update_network_baseline(self) -> None:
        ...
        for _router_id, metrics in self._router_behavior_metrics.items():
            if metrics.flagged:
                continue
    ...
    def _check_router_behavior(self, router_id: str) -> MisbehaviorReport | None:
        """Check if a router's behavior is anomalous."""
        from .messages import MisbehaviorEvidence, MisbehaviorReport, MisbehaviorType
        ...
        logger.warning(f"FLAGGED ROUTER {router_id[:16]}... for {misbehavior_type}: severity={severity:.2f}")
    ...
    def _detect_anomalies(self) -> dict[str, Any] | None:
        ...
        for failure_type, router_ids in failure_type_counts.items():
            ...
            if len(unique_routers) >= self.config.anomaly_threshold:
                ...
                logger.warning(
                    f"ECLIPSE ANOMALY DETECTED: {len(unique_routers)} routers experienced '{failure_type}' within {self.config.anomaly_window}s"  # noqa: E501
                )
    ...
    def track_ping_response(self, router_id: str, success: bool) -> bool:
        ...
        logger.warning(
            f"Ping timeout for router {router_id[:16]}... ({missed}/{self.config.missed_pings_threshold} missed)"
        )
