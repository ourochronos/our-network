...
class NodeClient:
    ...
    async def start(self) -> None:
        ...
        if self.recover_state_on_start and self.state_file:
            try:
                ...
                if recovered:
                    logger.info(
                        f"Recovered state: {len(self.pending_acks)} pending ACKs, {len(self.message_queue)} queued messages"  # noqa: E501
                    )
    ...
    async def _handle_router_failure(self, router_id: str) -> None:
        ...
        async def retry_messages() -> None:
            pending_for_router = [
                pending for msg_id, pending in self.pending_acks.items() if pending.router_id == router_id
            ]
    ...
    def get_traffic_analysis_mitigation_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "stats": {
                k: v for k, v in self._stats.items() if k.startswith(("batched", "batch_", "jitter", "constant_rate"))
            },
        }
