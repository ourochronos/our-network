...
if TYPE_CHECKING:
    ...
    from .node import FailoverState, RouterConnection
...
class ConnectionManager:
    ...
    def __init__(
        ...
        config: ConnectionManagerConfig | None = None,
        on_connection_established: Callable[..., Any] | None = None,
        on_connection_lost: Callable[..., Any] | None = None,
    ):
        """
    ...
    async def ensure_connections(self) -> None:
        ...
        while self.connection_count < self.config.target_connections and attempts < max_attempts:
            ...
            for router_id, state in self.failover_states.items():
                if state.is_in_cooldown():
                    ...
                    logger.debug(
                        f"Excluding router {router_id[:16]}... from discovery (in cooldown for {state.remaining_cooldown():.1f}s)"  # noqa: E501
                    )
        ...
        if self.connection_count < self.config.min_connections:
            logger.warning(
                f"Only {self.connection_count} connections established (minimum: {self.config.min_connections})"
            )
        ...
        for endpoint in router.endpoints:
            try:
                ...
                try:
                    ...
                    await ws.close()
                    await session.close()
                    raise ConnectionError("Identification timeout") from None
    ...
    async def close_connection(
        self,
        router_id: str,
        conn: RouterConnection | None = None,
    ) -> None:
    ...
    def get_connection(self, router_id: str) -> RouterConnection | None:
    ...
    def get_healthy_connections(self) -> list[RouterConnection]:
        ...
        return [conn for conn in self.connections.values() if not conn.websocket.closed]
